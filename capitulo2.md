...
## Conexión entre los mundos micro y macroscópicos
...
El evolucionar en el tiempo, el microestado del sistema cambia constantemente. De acuerdo con la hipótesis de igualdad de probabilidad a priori, después de un tiempo bastante largo, el sistema pasa tanto tiempo en uno de sus microestados compatibles como en cualquier otro. Sin embargo, el tiempo que el sistema pasa en cada mesoestado no es el mismo, sino que es proporcional al número de microestados compatibles con él. En otras palabras, al evolucionar, el sistema pasa más tiempo en el mesoestado más probable que en cualquier otro. En este sentido, resulta razonable suponer que el mesoestado más probable caracteriza al macroestado. De esta forma, la conexión entre los mundos mesoscópico y macroscópico la construiremos usando el mesoestado más probable.

De segunda ley sabemos que si el ensemble está aislado y en equilibrio termodinámico, el estado de equilibrio se caracteriza por tener la entropía máxima, comparado con el conjunto de estados termodinámicos compatibles con las restricciones macroscópicas.
...
$$
S^{*}=k \ln \left[\frac{N^{*}!}{\tilde{n}_{1}^{*}!\tilde{n}_{2}^{*}!\ldots \tilde{n}_{r}^{*}!} \omega_{1}^{\tilde{n}_{r}^{*}} \omega_{2}^{\tilde{n}_{1}^{*}} \ldots \omega_{r}^{\tilde{n}_{r}^{*}}\right]. \tag{2.1}
$$
...
La ecuación (2.1) permite calcular la entropía del ensemble a partir de los números de ocupación del mesoestado más probable. Todos los sistemas del ensemble están en equilibrio y tienen elmismo macroestado. Esto significa que la entropía está bien definida y que todos los sistemas tienen el mismo valor de entropía. Haciendo uso de este hecho, así como de la aditividad de la entropía, la entropía de cada sistema del ensemble se calcula como
...
$$
S=-k \sum_{i} p_{i} \ln p_{i} \omega_{i}. \tag{2.2}
$$
...
La definición de entropía dada por la ecuación 2.2 es inconsistente con la naturaleza física de la entropía. Esto se debe a que su valor depende de la forma en que se construye el sistema de celdas. Esto está en desacuerdo con el hecho de que la entropía, como toda vantidad termodinámica, es una propiedad del sistema cuyo valor es independiente de la forma en que se mide o se calcula. Este problema se resuelve considerando un ensemble formado por una cantidad no numerable de sistemas (tal que sus puntos representativos forman un continum en el espacio $\Gamma$ ) y tomando el límite al contínuo. Con esto, la ecuación 2.2 se convierte en

$$
S=-k \int_{\Omega} \rho(x) \ln \rho(x) d \gamma. \tag{2.3}
$$
...
Dada la forma en que se construye un ensemble canónico, no es necesaria ninguna condición además de la condición de normalización (2.4) para garantizar que el ensemble es un sistema aislado y en equilibrio. Así pues, la densidad de probabilidad de equilibrio den ensemble microcanónico es aquella que maximiza la entropía (2.4), sujeta a la condición de normalización (2.4). Este problema se resuelve empleando cálculo variacional. Para ello, se define el Lagrangiano:

$$
\Lambda=\int_{\Omega}[\rho(x) \ln \rho(x)-\alpha \rho(x)] d \gamma,
$$

donde $\alpha$ es un multiplicador de Lagrange indeterminado. De acuerdo a la teoría del cálculo variacional, la función $\rho(x)$ que maximiza (2.3) sujeta a la restricción (2.4) es aquella para la cual la variación de $\Lambda$ se anula:

$$
\delta \Lambda=\int_{\Omega}[\ln \rho(x)+1-\alpha] \delta \rho(x) d \gamma=0.
$$

$\delta \rho(x)$ es la variación de $\rho(x)$. $\delta \rho(x)$ es una variación arbitraria. En consecuencia, para que $\delta \Delta$ se anule se requiere que el integrando de la ecuación anterior sea igual con cero:
...
$C=\exp (\alpha-1)$ es una constante indeterminada. Su valor se encuentra requiriendo que $\rho(x)$ satisfaga la condición de normalización (2.4). Es fácil que después de hacer lo anterior, la densidad de probabilidad de equilibrio del ensemble microcanónico es:

$$
\rho(x)=\frac{1}{\Gamma}. \tag{2.5}
$$
...
Finalmente, sustituyendo (2.5) en (2.4) se obtiene la ecuación fundamental para cada uno de los sistemas del ensemble microcanónico:

$$
S\left(U, V, N_{i}\right)=k \ln \Omega\left(U, V, N_{i}\right). \tag{2.6}
$$

### Ensemble canónico

El ensemble canónico está constituido por sistemas con valores constantes e iguales de $V$ y $N_{i}$. Además, todos los sistemas del ensemble están en contacto térmico y en equilibrio con una fuente térmica a temperatura $T$. Como está definido, un ensemble canónico es un sistema en equilibrio termodinámico, pero no necesariamente está aislado. La razón de esto es que todos los sistemas intercambian energía con una fuente térmica que puede ser ajena al ensemble. Para que a pesar de esto el ensemble sea un sistema aislado, la fuente térmica debe ser parte del ensemble mismo. Una forma de concebir lo anterior es considerar que la fuente térmica con que cada sistema del ensemble está en contacto está formada por el resto de sistemas. De esta manera, los intercambios de calor se llevan a cabo al interior del ensemble y se garantiza que se trata de un sistema aislado.

Como se ha visto, para garantizar que el ensemble es un sistema aislado es necesario pedir explícitamente que la energía total del ensemble permanece constante. Esta condición se puede escribir como

$$
\int_{\Omega} \epsilon(x) \rho(x) d \gamma=\bar{\epsilon}. \tag{2.7}
$$

En la ecuación anterior $\epsilon(x)$ es la energía de un sistema cuyo punto representativo es $x$ y $\bar{\epsilon}$ es la energía promedio del ensemble (la energía total dividida por el número de sistemas del ensemble).

De acuerdo con la segunda ley de la termodinámica, la densidad de probabilidad de equilibrio del ensemble canónico es aquella que maximiza la entropía (2.3), sujeta a las restricciones (2.4) y (2.7). La teoría del cálculo variacional establece que esta densidad de probabilidad es la que optimiza al Lagrangiano $\Lambda$ definido como
...
$$
\rho(x)=\frac{1}{Z} e^{-\beta \epsilon(x)}. \tag{2.8}
$$

$Z=\exp (\alpha-1)$ y $\beta$ son constantes indeterminadas. El valor de $Z$ se determina usando la condición de normalización (2.4) como

$$
Z=\int_{\Omega} e^{-\beta \epsilon(x)} d \gamma. \tag{2.9}
$$

$Z$ se conoce como la función de partición.
En principio, debería ser posible determinar el valor de $\beta$ a partir de la restricción (2.7). Sin embargo, el álgebra es bastante complicada. Alternativamente, se sustituye la densidad de probabilidad dada por la ecuación (2.8) en la definición de entropía (2.3), de donde se obtiene que

$$
S=k \beta \bar{\epsilon}+k \ln Z. \tag{2.10}
$$

A continuación se usa la relación termodinámica $1 / T=(\partial S / \partial \bar{\epsilon})_{V, N_{i}}$, de donde finalmente resulta

$$
\beta=\frac{1}{k T}. \tag{2.11}
$$

En resumen, la densidad de probabilidad de equilibrio del ensemble canónico está dada por la ecuación (2.8), con $Z$ y $\beta$ determinadas respectivamente por las ecuaciones (2.9) y (2.11).

Puesto que en este caso las variables independientes son $T, V$ y $N_{i}$, que corresponden a la representación de la energía libre de Helmholtz, la ecuación fundamental se obtiene a partir de la ecuación (2.10) y la definición de $F$ como

$$
F\left(T, V, N_{i}\right)=-k T \ln Z\left(T, V, N_{i}\right). \tag{2.12}
$$
...
Del resultado anterior y la fórmula para la ecuación fundamental del ensemble microcanónico (2.6), tenemos que la ecuación fundamental del gas ideal en condiciones de energía $E$, volumen $V$ y número de partículas $N$ constantes es
...
$$
\frac{\mu}{T}=-k\left[\ln V+\frac{3}{2} \ln \left[\frac{4 \pi m E}{3 N}\right]\right]. \tag{2.22}
$$

Estas ecuaciones de estado son completamente compatibles con las ecuaciones de estado derivadas con el ensemble microcanónico: (2.15)-(2.17).
...
&=-\left.\frac{e^{-\beta H} p_{j}}{\beta}\right|_{-\infty} ^{\infty}+\frac{1}{\beta} \int_{-\infty}^{\infty} e^{-\beta H} d p_{j}. \tag{2.31}
\end{align*}
$$

El primer término se cancela en ambos límites dada la dependencia del hamiltoniano en $p_{j}$.
Así pues, reinsertando de la integral (2.31) en (2.30)

$$
\overline{p_{j} \frac{\partial K}{\partial p_{j}}}=\frac{1}{\beta} \int \frac{e^{-\beta H}}{Z} d \gamma=k T. \tag{2.32}
$$

Finalmente, de las ecuaciones (2.32) y (2.28) se sigue que

$$
\bar{K}=\frac{f}{2} k T. \tag{2.33}
$$

De manera análoga se demuestra para la energía potencial que

$$
\bar{V}=\frac{f}{2} k T. \tag{2.34}
$$

Las ecuaciones (2.33) y (2.34) constituyen el teorema de equipartición de la energía, el cual nos dice que si tanto la energía cinética como la energía potencial son respectivamente funciones homogéneas de segundo orden en las coordenadas y los momentos generalizados, entonces ambas se reparten equitativamente entre todos los grados de libertad.
